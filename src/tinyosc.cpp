...
uint64_t ntohll(uint64_t value) {
  return htonll(
		value); // Since the operation is symmetrical, use the same implementation
}
...
uint32_t TinyOSC::getBundleLength(bundle *b) { return b->bundleLen; }
...
char *TinyOSC::getAddress(message *o) { return o->buffer; }

char *TinyOSC::getFormat(message *o) { return o->format; }

uint32_t TinyOSC::getLength(message *o) { return o->len; }
...
void TinyOSC::writeBundle(bundle *b, uint64_t timetag, char *buffer,
                          const int len) {
...
uint32_t TinyOSC::vwrite(char *buffer, const int len, const char *address,
...
uint32_t TinyOSC::writeNextMessage(bundle *b, const char *address,
                                   const char *format, ...) {
  va_list ap;
  va_start(ap, format);
  if (b->bundleLen >= b->bufLen)
    return 0;
  const uint32_t i =
    vwrite(b->marker + 4, b->bufLen - b->bundleLen - 4, address, format, ap);
  va_end(ap);
...
uint32_t TinyOSC::writeMessage(char *buffer, const int len, const char *address,
                               const char *format, ...) {
  va_list ap;
  va_start(ap, format);
...
void TinyOSC::printMessage(message *osc) {
  ...
  for (int i = 0; osc->format[i] != '\0'; i++) {
    ...
    case 'f':
      printf(" %g", getNextFloat(osc));
      break;
    case 'd':
      printf(" %g", getNextDouble(osc));
      break;
    case 'i':
      printf(" %d", getNextInt32(osc));
      break;
    case 'h':
      printf(" %lld", getNextInt64(osc));
      break;
    case 't':
      printf(" %lld", getNextTimetag(osc));
      break;
    case 's':
      printf(" %s", getNextString(osc));
      break;
    case 'F':
      printf(" false");
      break;
